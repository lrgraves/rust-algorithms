use std::{fmt::{self, Error}, ops::{Add, Div, Mul}, vec};
...
impl fmt::Display for FittingError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            FittingError::InsufficientData => write!(f, "Insufficient data points for the requested polynomial degree"),
            FittingError::NumericalInstability => write!(f, "Numerical instability detected during computation"),
...
impl<T: Copy + Default> Matrix<T> {
    ...
    fn set(&mut self, row: usize, col: usize, value: T) -> Result<(), FittingError> {
        if row < self.rows && col < self.cols {
            ...
            Err(FittingError::InvalidDimensions(
                format!("Index ({}, {}) out of bounds for matrix {}x{}", row, col, self.rows, self.cols)
            ))
        }
    }
    ...
    fn multiply_by_second_matrix(&self, second_matrix: Matrix<T>)->Result<Matrix<T>, FittingError> {
        ...
        if self.cols != second_matrix.rows {
            return Err(FittingError::InvalidDimensions(
                format!("Cannot multiply {}x{} matrix with {}x{} matrix", 
        ...
        for row in 0..self.rows{
            for col in 0..second_matrix.cols{
                ...
                //    sum = sum +  self.get(row, k)? * second_matrix(k,col)?;
                //}
...
impl<T: Copy + PartialOrd + Add<Output = T> + Mul<Output = T> + Div<Output = T> + Default + std::ops::Neg<Output = T>> Matrix<T> {
    ...
    fn find_pivot(&self, column: usize, start_row: usize) -> Result<usize, FittingError> {
        if column >= self.cols || start_row >= self.rows {
            return Err(FittingError::InvalidDimensions(
                format!("Column {} or start row {} out of bounds", column, start_row)
            ));
        }
    ...
    fn swap_rows(&mut self, row_to_swap_to: usize, row_to_swap_from: usize) -> Result<&mut Self, FittingError> {
        if row_to_swap_to >= self.rows || row_to_swap_from >= self.rows {
            return Err(FittingError::InvalidDimensions(
                format!("Row indices {} and {} out of bounds", row_to_swap_to, row_to_swap_from)
            ));
        }
        ...
        for i in 0.. self.cols{
            temp_row[i] = self.get(row_to_swap_from, i).unwrap().clone();
    ...
    /// # Returns
    /// Self reference for method chaining
    fn eliminate_below(&mut self, pivot_row: usize, pivot_col: usize) -> Result<&mut Self, FittingError> {
        // Implementation would eliminate elements below pivot
        
        for i in pivot_row + 1..self.rows{
            let scale_factor = self.get(i, pivot_col).unwrap() / self.get(pivot_row, pivot_col).unwrap();

            for j in pivot_col .. self.cols{
                let source_value = self.get(pivot_row, j).unwrap();
                self.set(i, j, self.get(i,j).unwrap() + (source_value * scale_factor).neg());
            }
        }
    ...
    /// # Returns
    /// Self reference for method chaining
    fn eliminate_above(&mut self, pivot_row: usize, pivot_col: usize) -> Result<&mut Self, FittingError> {
        // Implementation would eliminate elements above pivot
        todo!()
...
impl<T: Copy + Add<Output = T> + Mul<Output = T> + Div<Output = T> + Default + PartialOrd + std::ops::Neg<Output = T>> PolynomialFit<T> {
    /// Creates a new polynomial fit instance
    /// 
    /// # Arguments
    /// * `input_points` - Vector of data points
    /// * `degree` - Degree of polynomial to fit
    fn new(input_points: Vec<Point<T>>, degree: usize) -> Result<Self, FittingError> {
...
let initial_matrix = Matrix{data:vec![vec![1,2,3], vec![4,5,6]]};
let result = initial_matrix.calculate_transposed_matrix();
